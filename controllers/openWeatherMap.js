...
module.exports = {
  ...
  byLocation: function(ctx, location){
    const url = `http://api.openweathermap.org/data/2.5/weather?lat=${location.latitude}&lon=${location.longitude}&APPID=${API_KEY}`
  ...
  weatherRequest: function(ctx, err, res, body){
    // If there is an error requesting.
      ...
      // The response.
      let response = 
        `The weather in ${city} is currently ${tempF}°F / ${tempC}°C and ${emoji} ${weatherDesc}.`
